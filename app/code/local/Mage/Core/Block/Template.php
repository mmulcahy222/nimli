...
class Mage_Core_Block_Template extends Mage_Core_Block_Abstract
{
    ...
    public function fetchView($fileName)
    {
        ...
        //MARK_ADDED_THIS
        if(Mage::registry('initial_setup') == false)
        {
            Mage::getSingleton('core/session')->setData('last_time', microtime(true));
            Mage::getSingleton('core/session')->setData('total_time', 0);
            Mage::register('initial_setup', true);
        }
        if($_GET['profiler'] == 1)
        {
            $last_time = Mage::getSingleton('core/session')->getData('last_time');
            $total_time = Mage::getSingleton('core/session')->getData('total_time');
            $time_since_last_block = microtime(true) - $last_time;
            Mage::getSingleton('core/session')->setData('last_time', microtime(true));
            Mage::getSingleton('core/session')->setData('total_time', $total_time + $time_since_last_block);
            $this->getLayout()->getBlock('bottom_of_page')->addText($fileName . "\t\t" . $time_since_last_block . ' ' . $total_time . "<br/>");
        }
        //
        //
        //        IN THIS ONE LINE OF REGISTRY CODE, IT IS FOR /varien/db/adapter/pdo/mysql.php debugging
        //        USING trace=1.
        //        
        //
        Mage::unregister("current_view");        
        Mage::register("current_view",$fileName);
